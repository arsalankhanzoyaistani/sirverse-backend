# backend/app.py
import os, sys, random, hashlib, threading, time
...
# ✅ FIXED: Database configuration for Railway
...
# Import DB init after app config
...
CORS(app, resources={
    r"/api/*": {
        "origins": ["https://your-netlify-app.netlify.app", "http://localhost:3000"],
        "methods": ["GET", "POST", "PUT", "DELETE", "OPTIONS"],
        "allow_headers": ["Content-Type", "Authorization"]
    }
}, supports_credentials=True)

# ✅ Fixed SocketIO setup
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if end - start > seconds:
                print(f"⚠️ Slow request: {func.__name__} took {end-start:.2f}s")
            return result
        return wrapper
    return decorator
...
# ✅ FIXED OTP SYSTEM
# ------------------------------------------------
def send_otp_email_background(email, username, otp):
    """Send OTP email in background thread to avoid blocking"""
    try:
...
def send_otp():
    ...
    if user and user.email != email:
        return jsonify({"error": "Username already taken with different email"}), 400
    ...
    email_thread = threading.Thread(target=send_otp_email_background, args=(email, username, otp))
...
def verify_otp():
    ...
    if user.otp_attempts and user.otp_attempts >= OTP_MAX_ATTEMPTS:
...
# ------------------------------------------------
# Serializers & Helpers (Keep your existing ones)
# ------------------------------------------------
...
# ------------------------------------------------
# API Endpoints (Keep your existing endpoints but add @timeout decorator)
# ------------------------------------------------
...
# SocketIO handlers (keep your existing ones)
...
# Add all your other existing endpoints with @timeout decorator
# Posts, Reels, Follow, Block, Report, etc. (copy from your current app.py)
...
# ... Add all your other existing endpoints here ...
...
def get_ai_history():
    uid = int(get_jwt_identity())
    msgs = AIHistory.query.filter_by(user_id=uid).order_by(AIHistory.created_at.asc()).all()
    return jsonify([
        {"id": m.id, "role": m.role, "text": m.message, "time": m.created_at.isoformat()}
        for m in msgs
    ]), 200
...
def save_ai_message():
    uid = int(get_jwt_identity())
    data = request.get_json() or {}
    msg = AIHistory(user_id=uid, role=data.get("role"), message=data.get("text"))
    db.session.add(msg)
    db.session.commit()
